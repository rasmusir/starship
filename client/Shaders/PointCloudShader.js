...
let vertex = `
    ...
    void main() {
        ...
        if (position.z < 0.41)
        {
            n = ( 1.0 + snoise(vec3(noisePos.x / 5.0, noisePos.y / 5.0, 1.0))) / 2.0;
            m = 1.0;
            a = 1.0 - (n*0.9);
        }

        gl_PointSize = position.z * 10.0 * n * m * size;

        p_col = vec4(1.0, 1.0, position.z*0.2 + 0.8, a);
...
let fragment = `
    ...
    void main() {
        vec4 texCol = texture2D(texture1, gl_PointCoord);
        //if ( texCol.a < 0.1 ) discard;
        gl_FragColor = texCol * p_col;
